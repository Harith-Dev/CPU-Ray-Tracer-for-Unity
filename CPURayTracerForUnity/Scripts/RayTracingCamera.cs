...
public class RayTracingCamera : MonoBehaviour
{
    ...
    public RawImage DisplayIn_RawImage;
    public bool DisableScript;        
    public bool UpdateFrame;
    public bool RecalculateCamRaysEveryFrame;    
    //Buffer
    Color32[] PixelsColor;
    ...
    //public float ExposureIntinsity = 1f;
    ...
    public float BrightnessVulue = 1.5f;
    ...
    public bool Gi_Effict_With_Sky_Texture;
    ...
    void Update()
    {
        if (DisableScript == false)
        {
            ...
            if (RayTracing == true || RecalculateCamRaysEveryFrame == true)
            {
                        ...
                        for (int y = 0; y < ScreenSize.y; y++)
                        {
                            ...
                            if (IsHit == true)
                            {
                                ...
                                if (IsHit == true)
                                {
                                    ...
                                    //ScreenSpaceRaysInfo[indexVulue].IsLight = hit.transform.GetComponent<RayTracingMaterial>().IsLight;
                                    //ScreenSpaceRaysInfo[indexVulue].LightIntensity = hit.transform.GetComponent<RayTracingMaterial>().LightIntensity;
                                    ...
                                    if (hit.transform.GetComponent<RayTracingMaterial>().IsTexture == true)
                                    {
                                        ...
                                        //NormalMap
                                        if (hit.transform.GetComponent<RayTracingMaterial>().NormalMap != null)
                                        {
                                            Texture2D NormalMap = hit.transform.GetComponent<RayTracingMaterial>().NormalMap;
                                            Vector2 tiling = hit.transform.GetComponent<RayTracingMaterial>().TilingNormal;
                                            Vector2 offset = hit.transform.GetComponent<RayTracingMaterial>().OffsetNormal;
                                            Vector2 adjustedUV = (UV * tiling);

                                            // Calculate pixel coordinates
                                            int UV_X = Mathf.RoundToInt(adjustedUV.x * NormalMap.width);
                                            int UV_Y = Mathf.RoundToInt(adjustedUV.y * NormalMap.height);
                                            //
                                            // Ensure UV_X and UV_Y are within valid bounds
                                            //UV_X = Mathf.Clamp(UV_X, 0, NormalMap.width - 1);
                                            //UV_Y = Mathf.Clamp(UV_Y, 0, NormalMap.height - 1);
                                            Color PixelNormalTextureColor = NormalMap.GetPixel(UV_X, UV_Y);
                                            Vector3 normalMapPixelDir = new Vector3(
                                            PixelNormalTextureColor.r * 2f - 1f,
                                            PixelNormalTextureColor.g * 2f - 1f,
                                            PixelNormalTextureColor.b * 2f - 1f
                                            );

                                            // PixelNormalTextureColor.b = 1f - PixelNormalTextureColor.b;
                                            //
                                            Vector3 normal = new Vector3(PixelNormalTextureColor.r * 2 - 1, PixelNormalTextureColor.g * 2 - 1, PixelNormalTextureColor.b);
                                            normal = normal.normalized;

                                            float NormalIntinsity = hit.transform.GetComponent<RayTracingMaterial>().NormalMapIntensity;
                                            Vector3 NormalVector = (hit.normal.normalized + (normalMapPixelDir.normalized)).normalized;
                                            //ScreenSpaceRaysInfo[indexVulue].HitNormal = (hit.normal.normalized + (normalMapPixelDir.normalized) * FixVulue).normalized;
                                            ScreenSpaceRaysInfo[indexVulue].HitNormal = NormalVector;
                                            Debug.DrawRay(RayHitPixelInfo[indexVulue].point, NormalVector, Color.blue);
                                            //RayHitPixelInfo[indexVulue].normal = (hit.normal + (normalMapPixelDir)).normalized;
                                            RayHitPixelInfo[indexVulue].normal = NormalVector;
                                            //
                                        }
                                        else
                                        {
                ...
                if (RayTracingShadow == true)
                {
                    if (StaticScene == true)
                    {
                        ...
                        //ChackAreaIfInShadowToSkipProccesRay

                        //CheckLightAngleToSkipJob            
                        //
                        for (int i = 0; i < PixelsCount; i++)
                        {
                            //Too Expinsve
                            //RaysWellHit[i] = Chack_Normal_and_Light_Angle(ScreenSpaceRaysInfo[i].HitNormal, LightDir[i]);
                            RaysWellHit[i] = Resulte_If_Weel_Hit[i];
                            //RaysWellHit[i] = true;
                            //Debug.Log(Chack_IF_Well_Hit);
                            //
                            if (RayHitPixelInfo[i].point == new Vector3(0, 0, 0) || RaysWellHit[i] == false)
                            {
                                ...
                                //PixelData[i].PixelsRaysHitObject++;
                            ...
                            else
                            {
                                ...
                                //PixelData[i].PixelsRaysHitLight++;
                                ...
                                for (int y = 0; y < ScreenSize.y; y++)
                                {
                                    ...
                                    if (ScreenSpaceRaysInfo[indexVulue].HitPoint != new Vector3(0, 0, 0))
                                    {
                                        ...
                                        if (ShadowRayHit_RT_Mat[indexVulue] != null)
                                        {
                                            ...
                                            {
                                                //----------Make Light Have no Colider to make it fast----------
                                                ...
                                                if (ShadowRayInfo_RT_Mat.IsLight == true)
                                                {
                                                    ...
                                                    //Vector3 NormalVector = ScreenSpaceRaysInfo[indexVulue].HitNormal;
                ...
                if (RayTracingShadow == true)
                {
                    ...
                    for (int i = 0; i < PixelsColor.Length; i++)
                    {
                        //PixelData[i].PixelsRaysHitLight / PixelData[i].RaysShadowSamplesProccesed
                        if (RayHitPixelInfo[i].transform != null)
                        {
                            ...
                            {
                                //PixelsColor[i] = Color32.Lerp(PixelsColor[i], Color.black, PixelData[i].PixelsRaysHitLight / 0.0001f);
                            }
                            else
                            {
                                PixelsColor_Shadow[i] = Color32.Lerp(Color.black, PixelsColor_Shadow[i], PixelData[i].PixelsRaysHitLight / DividedVulueForShadow);
                            }
                            //PixelsColor[i] = Color32.Lerp(Color.black, PixelsColor[i], PixelData[i].PixelsRaysHitLight / DividedVulueForShadow);
                        }
                    }
                ...
                if (Global_ilomimtion == true)
                {
                    ...
                    //int RandomYStarting = Random.Range(0, PixelsSkipPathTracing + 1);
                    //Global Illumntion
                            ...
                            if (ScreenSpaceRaysInfo[i].IsHit == true && PixelsWellRender[i] == true)
                            {
                                ...
                                if (hits[i].transform != null)
                                {
                                    if (hits[i].transform.GetComponent<RayTracingMaterial>().GlassIntinsty == 0f)
                                    {
                                        ...
                                        //Debug.DrawRay(hits[i].point, Bounce2Dir, Color.blue);
                                    ...
                                    else
                                    {
                                        ...
                                        //Debug.DrawRay(hits[i].point, Bounce2Dir * 15f, Color.green);
                    ...
                    for (int i = 0; i < PixelsColor.Length; i++)
                    {
                        if (ScreenSpaceRaysInfo[i].IsHit == true && PixelsWellRender[i] == true)
                        {
                            ...
                            if (BounceTolight == true)
                            {
                                ...
                                //Debug.DrawLine(hit.point, hitBounce.point, Color.blue);
                            }
                            if (HitResulteBounce == true)
                            {
                                ...
                                if (RT_Mat_BounceObjectHited.IsLight == true)
                                {
                                    ...
                                    //Debug.Log("HitLight !! =" + RT_Mat_BounceObjectHited.IsLight);
                                    //Debug.DrawLine(hit.point, hitBounce.point, Color.blue);
                                }
                                else
                                {
                            ...
                            if (BounceTolight == true)
                            {
                                ...
                                if (hits[i].transform != null && PixelsWellRender[i])
                                {
                                    if (hits[i].transform.GetComponent<RayTracingMaterial>().GlassIntinsty != 0 || hits[i].transform.GetComponent<RayTracingMaterial>().Smoothness != 0)
                                    {
                                        ...
                                        {                                            
                                            ...
                                            {
                                                //Debug.DrawLine(hitBounce.point, hits[i].point, Color.red);
                                            }
                                        }
                                        else
                                        {
                                ...
                                {
                                    //float CurrentFramesEdited = (CurrentFrame / 0.5f);
                            ...
                            //Color.RGBToHSV(PixelsColor_PTRefracte[i], out float H_Refracte, out float S_Refracte, out float V_Refracte);
                            //PixelsColor_Global_Illumination[i] = Color.LerpUnclamped(Gi_Combine_Objecte_Hited, PixelsColor_PTRefracte[i], V_Refracte * CuasticesMultpler);
                ...
                if (RayTracingReflaction == true)
                {
                    ...
                    if (RenderShadow == true)
                    {
                        ...
                        for (int i = 0; i < PixelsColor.Length; i++)
                        {
                            if (PixelWellReflacte[i] == true)
                            {
                                //(PointsLight[RandomHitPoint[i]] - RayHitPixelInfo[i].point).normalized;
                                ...
                                //Debug.DrawRay(hits[i].point, PointLightDir);
            ...
            //
            //int DebugModeGBuffer_int = Debuging_GBuffer(GBufferDebuging);
                ...
                if (DebugMode_int == 0)
                {
                    if (EmissivePixels[i].r == BlackCol32.r && EmissivePixels[i].g == BlackCol32.g && EmissivePixels[i].g == BlackCol32.g)
                    {
                        ...
                        if (RayHitPixelInfo[i].transform != null)
                        {
                            ...
                            {
                                //Color_Lighting = Color.LerpUnclamped(Color.black, PixelsColor[i], V );
                            ...
                            if (RayTracingReflaction == true)
                            {
                                ...
                                if (RayHitPixelInfo[i].transform != null)
                                {
                                    ...
                                    if (RayHitPixelInfo[i].transform.GetComponent<RayTracingMaterial>().GlassIntinsty == 0)
                                    {
                                        //float FresnalA = FresnalAngle(RayHitPixelInfo[i], SSRayDir[i]);
                                        //ReflactionAndRefraction = Color.LerpUnclamped(PixelsColor_Reflaction[i], Color_Lighting, 1f - RayHitPixelInfo[i].transform.GetComponent<RayTracingMaterial>().Smoothness);
                                        if (RayHitPixelInfo[i].transform.GetComponent<RayTracingMaterial>().ClearCoat)
                                        {
                                            ...
                                            //Color.RGBToHSV(Color_Lighting, out float H6, out float S6, out float V6);
                                            //float FresnalA = FresnalAngle(RayHitPixelInfo[i], SSRayDir[i]);
                                            ...
                                            //Debug.Log(ClearCoatFix);
                                            Color ColorClearCoatReflacte = Color.HSVToRGB(H5, S5, Mathf.Pow(V5, ClearCoatFix));
                                            //ReflactionAndRefraction = Color.LerpUnclamped(PixelsColor[i], PixelsColor_Reflaction[i], V * FixVulue);
                                            ReflactionAndRefraction = ColorClearCoatReflacte;

                                            //ReflactionAndRefraction = Color.LerpUnclamped(ColorClearCoatReflacte, Color_Lighting,1f - RayHitPixelInfo[i].transform.GetComponent<RayTracingMaterial>().Smoothness);
                        ...
                        //Color Color_Lighting;
                        //Color_Lighting = Color.Lerp(PixelsColor_Shadow[i], PixelsColor_Global_Illumination[i], 0.5f);
                        //PixelsColor[i] = Color_Lighting;
                    }
            ...
            if (Denoiser == true)
            {
                //int PixelsCount = ScreenSize.x * ScreenSize.y;
                if (StaticScene == true)
                {                    
    ...
    public struct DenoiserJob : IJobParallelFor
    {
        ...
        //[ReadOnly] public NativeArray<Color32> PixelsRead;
        [WriteOnly] public NativeArray<Color32> PostProccesImage;
        [ReadOnly] public float BrightnessVulue;
        //[ReadOnly] public NativeArray<PixelData> PixelsData;
        //[ReadOnly] public NativeArray<bool> RealTimePreviewShadow;
        //[ReadOnly] public NativeArray<RaycastHit> RayHitPixelInfo;        
    ...
    Color32 ProccesGiRays_Global_Illumination(bool IsHit, RaycastHit hit, bool IsHitBounce, RaycastHit HitBounce, Vector3 SSHitPoint, Vector3 SSHitNormal, Vector3 PointLightPos, Color32 LightCol, bool Bounce,Vector3 GiRayDir)
    {
        ...
        //RayTracingMaterial RT_Mat_OBJ_Hited_Bounce;
        ...
        if (IsHit == true)
        {
            if (Bounce == false)
            {
                ...
                //Vector3 LightDir = LightDirction;
                //float CalcolateRefracteLight = Vector3.Angle(SSHitNormal,)
                //Resulte = Color.Lerp(FinelResulte,Color.black,)
            }
            else
            {
                RT_Mat_OBJ_Hit = OBJ.GetComponent<RayTracingMaterial>();
                //RT_Mat_OBJ_Hited_Bounce = OBJBounce.GetComponent<RayTracingMaterial>();
                if (RT_Mat_OBJ_Hit.EmissiveIntinsity == 0)
                {
    ...
    Vector3 GetLightPoints(int i, GameObject OBJ, bool LowVerticesMode)
    {
        ...
        {
            int selectedTriangle = Random.Range(0, Tringles.Length / 3) * 3; // اختيار مثلث عشوائي
            Vector3 vertex1 = Vertices[Tringles[selectedTriangle]];         // الرأس الأول
            Vector3 vertex2 = Vertices[Tringles[selectedTriangle + 1]];     // الرأس الثاني
            Vector3 vertex3 = Vertices[Tringles[selectedTriangle + 2]];     // الرأس الثالث
        ...
        //Vector3 WorldPosVer = transform.TransformPoint(newVector);

        return newVector;
    }

    int RandomGenorator(int x, int y)
    {
        int PixelsInScreen = (((int)ScreenSize.x) * ((int)ScreenSize.y));
        int RandomVulue = x / ((int)ScreenSize.x);
        float RandomLightPointUsed = Mathf.Lerp(0.0f, PointsLight.Length, RandomVulue);
        int FinelBlueNoiserResulte = ((int)RandomLightPointUsed);
        return FinelBlueNoiserResulte;
    ...
    Color32 SkyRayDir(Vector3 Dir, Texture2D SkyTexture)
    {
        ...
        Dir.Normalize();
        float theta = Mathf.Atan2(Dir.z, Dir.x); // azimuthal angle (around Y-axis)
        float phi = Mathf.Acos(Dir.y); // polar angle (from Y-axis)

        // Map spherical coordinates to UV space
        float u = (theta + Mathf.PI) / (2 * Mathf.PI); // Map [-π, π] to [0, 1]
        float v = phi / Mathf.PI; // Map [0, π] to [0, 1]

        // Convert UV to pixel coordinates by multiplying by texture width and height
    ...
    bool RaysMothod_B(RaysMothod raysMothod)
    {
        switch (raysMothod)
        {
            case RaysMothod.RayCast:
                return true;
            case RaysMothod.LineCast:
                return false;
            default: return true;
        }
    }
